...
def get_movie_suggestions_actor(user):
    ...
    for personId in actors_info.index:
        ...
        output_list.append({'actor': full_name,
                            'movies': suggested_movies_actor})
    return output_list
...
class SuggestionsCluster(APIView):

    permission_classes = (permissions.IsAuthenticated, )

    def get(self, request):
        # user = User.objects.filter(username='test_ben')[0]
        user = request.user
        ...
        for cluster_id in cluster_ids:
            ...
            data_list.append(cluster_dict)
        output = OutputObject(status='normal',
                              data=data_list)
        return Response(data=output.get_output_dict())


class SuggestionsActor(APIView):
