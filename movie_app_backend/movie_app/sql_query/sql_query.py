...
class AutocompleteAllMovies(RawSQLQuery):

    def __init__(self):
        super().__init__()
        self.read_sql(
            file='movie_app/sql_query/autocomplete_all_movies.sql'
        )
...
class AutocompleteRatedMovies(RawSQLQuery):

    def __init__(self):
        super().__init__()
        self.read_sql(
            file='movie_app/sql_query/autocomplete_rated_movies.sql'
        )

    def build_query(self, term, user_id):
        term = str.lower(term)
        self.query = self.query.replace('-- TERM',
                                        "'%" + str(term) + "%'")
        self.query = self.query.replace('-- USER-ID',
                                        str(user_id))
